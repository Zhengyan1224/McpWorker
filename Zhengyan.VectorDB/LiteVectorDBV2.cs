...
public class LiteVectorDBV2 : IVectorDB
{
    ...
    public SmallWorld<CVector, float>? World { get; private set; }
    ...
    public string VectorFilePath => Path.Combine(StorageDirectoryPath, VectorFileName);
    ...
    internal float UnitCompute(CVector a, CVector b)
    {
        ...
        return CosineDistance.ForUnits(a.Vector, b.Vector);
    }
    internal float SIMDUnitCompute(CVector a, CVector b)
    {
        if (a == null || b == null || a.Vector == null || b.Vector == null)
            return float.MaxValue;
    ...
    internal float Compute(CVector a, CVector b)
    {
        if (a == null || b == null || a.Vector == null || b.Vector == null)
            return float.MaxValue;
        return CosineDistance.NonOptimized(a.Vector, b.Vector);
    }
    internal float SIMDCompute(CVector a, CVector b)
    {
        if (a == null || b == null || a.Vector == null || b.Vector == null)
            return float.MaxValue;
        return CosineDistance.SIMD(a.Vector, b.Vector);
    }

    private LiteVectorDBV2(string storageDirectoryPath, IDataStorage dataStorage, int embeddingSize = 0, bool forUnits = false, bool freeVector = false)
    {
        ...
        if (this.ForUnits)
            World = new SmallWorld<CVector, float>(Vector.IsHardwareAccelerated ? SIMDUnitCompute : UnitCompute, DefaultRandomGenerator.Instance, @params);
        else
            World = new SmallWorld<CVector, float>(Vector.IsHardwareAccelerated ? SIMDCompute : Compute, DefaultRandomGenerator.Instance, @params);
        IndexTable = new();
    ...
    public IVectorDB AddItems(IEnumerable<(float[], byte[])>? items)
    {
        ...
        var ilist = World?.AddItems(vectors) ?? new int[0];
    ...
    {
        var results = World?.KNNSearch(new CVector(query, ForUnits), k, item => !item.Deleted);
        ...
        foreach (var it in results?.Zip(contents) ?? new List<(SmallWorld<CVector, float>.KNNSearchResult, byte[])>())
        {
            searchResult.Add(new IVectorDB.SearchResult(it.First.Id, it.First.Item.Vector, it.First.Distance, it.Second));
        }
    ...
    public void Delete()
    {
        try
        {
            ...
            if (File.Exists(this.VectorFilePath))
                File.Delete(this.VectorFilePath);
    ...
    private void SaveGraph(string graphFilePath, string vectorFilePath, SmallWorld<CVector, float>? world)
    {
        try
        {
            // indexRWLock.EnterWriteLock();
            using (Stream serializeStream = File.Open(graphFilePath, FileMode.Create, FileAccess.Write))
            {
                world?.SerializeGraph(serializeStream);
            }

            // 保存向量集合
            using (Stream serializeStream = File.Open(vectorFilePath, FileMode.Create, FileAccess.Write))
            {
                Serializer.Serialize(serializeStream, world?.Items.ToArray() ?? Array.Empty<CVector>());
            }
        }
        catch (Exception e)
        {
    ...
    private void SaveGraph() => SaveGraph(GraphFilePath, VectorFilePath, World);


    public static IVectorDB Load(string storageDirectoryPath, bool forUnits = false, bool freeVector = false)
    {
        LiteVectorDBV2 vectorDB = new LiteVectorDBV2(storageDirectoryPath, new LocalDiskDataStorage(storageDirectoryPath), forUnits: forUnits, freeVector: freeVector);
        ...
        if (!File.Exists(vectorDB.VectorFilePath))
            throw new FileNotFoundException($"Could not find file '{vectorDB.VectorFilePath}'.");

        vectorDB.World = vectorDB.ReadGraph(vectorDB.GraphFilePath, vectorDB.VectorFilePath);
    ...
    private SmallWorld<CVector, float> ReadGraph(string graphFilePath, string vectorFilePath)
    {

        // 读取向量集合
        CVector[] vectors = Array.Empty<CVector>();
        using (Stream deserializeStream = File.OpenRead(vectorFilePath))
        {
            vectors = Serializer.Deserialize<CVector[]>(deserializeStream);
        }

        using (Stream deserializeStream = File.OpenRead(graphFilePath))
        {
            if (this.ForUnits)
            {
                // var (world, itemsNotInGraph) = SmallWorld<CVector, float>.DeserializeGraph(vectors, Vector.IsHardwareAccelerated ? SIMDUnitCompute : UnitCompute, DefaultRandomGenerator.Instance, deserializeStream);
                var world = SmallWorld<CVector, float>.DeserializeGraph(vectors, Vector.IsHardwareAccelerated ? SIMDUnitCompute : UnitCompute, DefaultRandomGenerator.Instance, deserializeStream);
                return world;
            }
            else
            {
                // var (world, itemsNotInGraph) = SmallWorld<CVector, float>.DeserializeGraph(vectors, Vector.IsHardwareAccelerated ? SIMDCompute : Compute, DefaultRandomGenerator.Instance, deserializeStream);
                var world = SmallWorld<CVector, float>.DeserializeGraph(vectors, Vector.IsHardwareAccelerated ? SIMDCompute : Compute, DefaultRandomGenerator.Instance, deserializeStream);
                return world;
            }
        }
